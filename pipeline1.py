...
def read_data() -> DataFrame:
    ...
    #return read_csv(INPUT_FILENAME, compression="gzip")
...
def etl_transformation(data:DataFrame) -> DataFrame:
    ...
    # lambda apply
    output["date_math"] = output["date"].apply(lambda x: (datetime.utcnow() - datetime.strptime(x, "%d/%m/%Y %H:%M:%S %p")).seconds * 123456789.545)
